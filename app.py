    ...
    if 'username' in session:
        if request.method == 'POST':
            ...
            if request.form['button'] == 'Classify':
                ...
                # Load the Word2Vector model
                advertisew2v = Word2Vec.load("tkAdvertise_w2v.model")
                advertisew2v_wv = advertisew2v.wv

                # Generate vector representation of the tokenized data
                advertisew2v_dvs = gen_docVecs(advertisew2v_wv, [tokenized_data])

                # Load the LR model
                pkl_filename = "Advertisew2v_LR.pkl"
                with open(pkl_filename, 'rb') as file:
                    model = pickle.load(file)

                # Predict the label of tokenized_data
                y_pred = model.predict(advertisew2v_dvs)
                y_pred = y_pred[0]
